...
public class DiscountWebCall extends AsyncTask{
	String couponcode;
	
	public DiscountWebCall(String couponcode){
	...
	protected Object doInBackground(Object... params) {
		String result = null;
		String url = "http://10.0.2.2:8000/payment?item=discount&name=coupon&number="+couponcode;
		
		System.out.println("Discount url: "+url);
						...
						if(eventType == XmlPullParser.START_DOCUMENT){
							...
							while((xpp.nextTag() != XmlPullParser.END_TAG) && (!xpp.getName().equals("Items"))){
								...
								if(paymenttag.equals("Payment")){
				            	 ...
				            	 xpp.nextTag();
				            	 String subtagcoupon = xpp.getName();
				            	 if(subtagcoupon.equals("couponAmount")){
				            		 String text = xpp.nextText();
				            		 System.out.println(text);
				            		 WebPayment.paymentcoupon = text;
				            	 }	
				            	 xpp.nextTag();
				            	 xpp.nextText();
				            	 System.out.println("one: "+xpp.getName());
				            	 xpp.nextTag();
				            	 xpp.nextText();
				            	 System.out.println("two: "+xpp.getName());
				            	 xpp.nextTag();
				            	 String subtagtax = xpp.getName();
				            	 System.out.println("subtagtax: "+subtagtax);
				            	 if(subtagtax.equals("taxafterAppliedDiscount")){ //tax amount
				            		 String text = xpp.nextText();
				            		 System.out.println(text);
				            	 ...
				            	 if(subtagtotal.equals("totalAmountAfterDiscount")){ //tax amount
				            		 String text = xpp.nextText();
				            		 System.out.println(text);
